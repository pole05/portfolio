...
<header>
	...
	<table class=tb2 align="center" width="800">	
		<tr>
			...
			<a href=menuList.do?order=starttime>최신순</a> / &nbsp;
			<a href=menuList.do?order=heart_cnt>인기순</a> / &nbsp;
			<a href=menuList.do?order=comment_cnt>댓글순</a></td>
			</c:if>
			<c:if test="${category != null}">
				...
				<table width=400>
					<tr>
						<td width=200 rowspan=2 class=poster>
							<div id="postercell" style="display:table; width:200px; height250px; overflow: hidden;">
							<span style="display: table-cell; text-align: center; vertical-align: middle; overflow: hidden;">
								<a href="menuDetail.do?no=${list.no}">
								<c:choose>
									<c:when test="${list.p_poster_s != '' && list.p_poster_s != null && fn:contains(list.p_poster_s, 'http')}">
										<img src="${list.p_poster_s}" width="200" height="250" style="max-width:200px; vertical-align: middle; overflow: hidden;">
									</c:when>
									<c:otherwise>
										<img src="menuDownload.jsp?no=${list.no}&p_no=1" width=200 style="vertical-align: middle; overflow: hidden;">
									</c:otherwise>
								</c:choose>
								</a>
							</span>
