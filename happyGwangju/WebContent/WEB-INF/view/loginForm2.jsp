...
<link href="/css/loginForm2.css" rel="stylesheet"/>
<script>
	$(document).ready(function(){
		...
		if(sectionWidth >= 1180){
			$("<br id='loginBR'>").insertBefore('#searchID');
			$('#loginSubmit').width('76%');
			$('#searchID').width('76%');
		}else{
			$("#loginBR").remove();
			$('#loginSubmit').width('38%');
			$('#searchID').width('38%');
		}
		...
		$('#loginForm').on('click','#searchID',function(){
...
<header>
	...
	<div id='loginPage'>   
	<img id='loginIcon' src='/img/icon_delete.png'/><br>
		<form id='loginForm' method="post" action="login.do">
			...
			<input type='button' id='searchID' value='계정 찾기'>
		</form>
	</div>
