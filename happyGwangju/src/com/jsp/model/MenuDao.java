...
//			dto.setCategory_s(rs.getString("CATE_S"));
	...
	public List<MenuDto> findCategoryMenu(String text, int startRow, int category) {	// 카테고리별 게시글 검색
		Connection con = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
	...
	public int findCategoryCount(String text, int category) {	// 카테고리별 검색 totalCount
		Connection con = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
	...
	public int findMenuForAutoSubmit(String text) {	// AutoSubmit을 할때 기존글과 중복되지 않게하기 위함
		Connection con = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		sql.append("SELECT NO FROM MENU WHERE NAME = ?;");
	...
	public List<MenuDto> mobileSelectBoard() { // 모바일... 어플에서 전체 리스트 보여주기
		// startRow: x페이지일 때 게시물 시작 번호
		Connection con = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		try {
			...
			sql.append("	END AS CATE_S, ");
			sql.append("	P.*, M.* ");
			sql.append("	FROM MENU_POSTER P JOIN MENU M ");
			sql.append("	ON P.NO = M.NO ");
			sql.append("	WHERE (CATEGORY = 1 OR CATEGORY = 2 OR CATEGORY = 3 OR CATEGORY = 5) AND P.P_NO = 1 ");
			sql.append("	ORDER BY M.NO DESC");			
